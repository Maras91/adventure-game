...
class LevelService {
    fun levelUp(statsToUp: Map<String, Int>, player: Player) {
        val namesStatsToUp = statsToUp.keys.filter { StatisticsName.values().map { name -> name.attributeName }.contains(it)}
        val statsUp = StatsUp(
            namesStatsToUp.associate{ (StatisticsName.getAttributeByName(it)) to (statsToUp[it] ?: 0) }
        )
